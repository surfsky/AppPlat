...
{
    public delegate void TaskDelegate(Task task, string info);
    ...
    public class ScheduleEngine
    {
        ...
        // 构造函数
        //----------------------------------------
        public ScheduleEngine(TaskConfig config)
        {
            this.Config = config;
        }
        ...
        public void Start()
        {
            ...
            if (!_configFile.IsNullOrEmpty())
            {
                try
                {
                    this.Config = new TaskConfig(_configFile);
                }
                catch (Exception ex)
                {
        ...
        private bool CheckConfig()
        {
            ...
            {
                if (task.Schedule == null || task.TaskRunner == null)
                    return false;
            }
            return true;
        }
        ...
        protected bool ProcessTask(Task task, DateTime dt)
        {
            // 当它不存在
            if (!task.Enable)
                return true;
            ...
            // 成功状态，且未到下次运行时间
            if (task.Status == TaskStatus.Success && !NeedToRunAgain(task.LastRunDt, task.Success, dt))
                return true;

            // 失败状态，但又要开始一个周期了
            if (task.Status == TaskStatus.Failure && NeedToRunAgain(task.LastRunDt, task.Success, dt))
            {
            ...
            if (
                (task.Status == TaskStatus.Waiting && NeedToRunAgain(task.LastRunDt, task.Success, dt))
             || (task.Status == TaskStatus.Success && NeedToRunAgain(task.LastRunDt, task.Success, dt))
             || (task.Status == TaskStatus.Failure && NeedToRunAgain(task.LastRunDt, task.Failure, dt) && task.Failure.TryTimes < task.Failure.MaxTimes)
            )
            {
